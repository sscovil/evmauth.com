...
pub fn encode_beacon_proxy_deploy(beacon: Address, init_data: Bytes) -> Bytes {
    let hex_str = BEACON_PROXY_BYTECODE_HEX
        .trim()
        .strip_prefix("0x")
        .unwrap_or(BEACON_PROXY_BYTECODE_HEX.trim());
    let creation_code = hex::decode(hex_str).expect("invalid BeaconProxy bytecode hex in artifact");
