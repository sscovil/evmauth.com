...
#[utoipa::path(
    ...
    path = "/me/wallets",
    responses(
        (status = 200, description = "List of person's app wallets", body = Vec<EntityAppWalletResponse>),
        (status = 500, description = "Internal server error")
    ),
    tag = "entity_app_wallets"
)]
pub async fn list_my_wallets(
    State(_state): State<AppState>,
) -> Result<Json<Vec<EntityAppWalletResponse>>, ApiError> {
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Person app wallet found", body = EntityAppWalletResponse),
