    ...
    pub fn merge_specs(&self, specs: HashMap<String, Value>, api_config: &ApiConfig) -> Value {
        ...
        for (service_name, mut spec) in specs {
            ...
            if let Some(paths) = spec.get_mut("paths") {
                if let Some(merged_paths) = merged["paths"].as_object_mut() {
                    if let Some(spec_paths) = paths.as_object() {
            ...
            if let Some(components) = spec.get_mut("components") {
                if let Some(schemas) = components.get_mut("schemas") {
                    if let Some(merged_schemas) = merged["components"]["schemas"].as_object_mut() {
                        if let Some(spec_schemas) = schemas.as_object() {
            ...
            if let Some(tags) = spec.get_mut("tags") {
                if let Some(merged_tags) = merged["tags"].as_array_mut() {
                    if let Some(spec_tags) = tags.as_array() {
                        for tag in spec_tags {
                            // Skip health tag
                            if let Some(tag_obj) = tag.as_object() {
                                if let Some(name) = tag_obj.get("name") {
                                    if name.as_str() == Some("health") {
                                        continue;
                                    }
                                }
                            }
                            ...
                            if let Some(tag_obj) = tag.as_object_mut() {
                                if let Some(name) = tag_obj.get("name") {
                                    ...
                                    tag_obj
                                        .insert("name".to_string(), Value::String(prefixed_name));
                                }
                            }
